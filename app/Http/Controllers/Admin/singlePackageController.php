...
class singlePackageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($check) {
            ...
            return view('singlepackage.index', compact('page_name', 'hotelRecord', 'Categories', 'couponRecord'));
        }
    }
    ...
    public function get_list(Request $request)
    {
        ...
        if ($check) {
            if ($request->ajax()) {
                if ($request->title != '') {
                    $keyword = $_GET['title'];
                    $records = Package::where('title', 'like', '%'.$keyword.'%')->where('type', 'single')->paginate(15);
                } else {
                    $records = Package::where('type', 'single')->paginate(15);
                }
                $response['success'] = true;
                $response['html'] = view('singlepackage.list', ['records' => $records])->render();
    ...
    public function details(Request $request, $id)
    {
        ...
        if ($check) {
            $record = Package::where(['id' => $id])->first();
            if (! empty($record)) {
                $response['html'] = view('singlepackage.details', ['records' => $record])->render();
                $response['success'] = true;
            } else {
                $response['success'] = false;
            }
